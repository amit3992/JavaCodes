...
public class dijkstra {
	...
	private static final int N_INFINITY = -100000001;
	...
	dijkstra(int n) {
		...
		vtx = new Vertex[this.n];
		
		// Build all vertices
		for(int i = 0; i < this.n; i++) {
			vtx[i] = new Vertex(i);
			allVertices.put(i, vtx[i]);
		}
	}
  ...
  private int distance(Collection<Vertex> collection, int source, int dest) {
      ...
      if(distanceMap.get(destVertex) <= N_INFINITY|| distanceMap.get(destVertex) >= INFINITY) {
      	distance = -1;
      }
      else {
